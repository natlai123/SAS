...
/* Plot the moment-ratio diagram for a given data set and a
...
%macro PlotMRDiagram(DS, annoDS, Transparency=0);
   /* Given a data set that contains variables KURT and SKEW, 
      add the FULLKURTOSIS variable and labels for the three variables. */
data &ds;
   set &ds;
   FullKurt = Kurt+3;
   label Kurt="Excess Kurtosis" FullKurt="Full Kurtosis" Skew="Skewness";
run;
...
/* Display a moment-ratio diagram in the background of a scatter
   plot of the sample skewness and kurtosis for simulated samples. */
title "Moment-Ratio Diagram";
%PlotMRDiagram(MomentsGamma, Anno, Transparency=0.5);
