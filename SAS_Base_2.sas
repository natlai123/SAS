...
• Using IF-THEN statements with the ELSE statement causes SAS to execute IFTHEN
...
data work.admin work.stock work.purchasing;
	...
	select(Department);
		...
		when ('Stock & Shipping') otput work.stock;
...
run;
                                        ...
                                        /*Extracting the summary report by project and dept*/
...
/*M2 different from M1: Method using Little SAS book*/
...
formated input
   ...
   @n = colum-pointer control 
   +n = relative-pointer control   
   / = relative line pointer
   #n = absolute line pointer 
   @ = line=hold specifier
   @@ = double trailing @ 
   input
